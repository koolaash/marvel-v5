    ...
    async run(client, message, args) {
        ...
        if (kickMember.id === message.member.id) {
            return message.reply({
                embeds: [
                    new MessageEmbed({
                        description: `${client.emoji.fail}| **Cannot unmute yourself!**`,
                        color: client.embed.cf
                    })
                ]
            })
            ...
            if (message.member.roles.highest.position <= kickMember.roles.highest.position) {
                return message.reply({
                    embeds: [
                        new MessageEmbed({
                            description:
                                `${client.emoji.fail}| Your Role isn't High Enough to unmute **\`\`${kickMember.user.tag}\`\`**`,
        ...
        if (
            ...
            return message.reply({
                embeds: [
                    new MessageEmbed({
                        description:
                            `${client.emoji.fail}| My Role isn't High Enough to unmute **\`\`${kickMember.user.tag}\`\`**`,
                        color: client.color.cf
                    })
                ]
            })
        ...
        if (kickMember.permissionsIn(message.channel).has("ADMINISTRATOR")) {
            return message.reply({
                embeds: [
                    new MessageEmbed({
                        description: `${client.emoji.fail}| **Cannot unmute this user because he has admin permissions!**`,
                        color: client.embed.cf
                    })
                ]
            })
